        ...
        if "openapi" in low or "swagger" in low:
            ...
            for name, scheme in (sec or {}).items():
                if isinstance(scheme, dict) and scheme.get("type") == "oauth2":
                    ...
                    hits.append({"scheme_name": name, "flows": flow_names})
...
def scan_code_patterns(root: str):
    ev = []
    pat = re.compile("|".join(OAUTH_JS_PATTERNS))
    for path in walk_files(root):
        ext = os.path.splitext(path)[1].lower()
        if ext not in TEXT_EXTS:
            continue
...
def pick_component_name(root: str, hints: dict) -> str:
    if hints.get("component", {}).get("name"):
        return hints["component"]["name"]
...
def main():
    ...
    hints = load_hints(args.hints, repo)
    component_name = pick_component_name(repo, hints)
    component_id = (
        re.sub(r"[^a-zA-Z0-9_-]+", "-", component_name).strip("-").lower() or "main-app"
    )
    ...
    components = [
        {
            "id": component_id,
            "name": component_name,
            "type": hints.get("component", {}).get("type", "Application"),
            "tech": hints.get("component", {}).get("tech", ""),
            "owners": hints.get("component", {}).get("owners", []),
            "evidence": [],
        }
    ]
    #print(components)
        ...
        components.append(
            {
                ...
                "metadata": {
                    ...
                    "libraries": {"python": [], "javascript": js_hits},
                    "openapi_oauth_schemes": openapi_hits,
                },
                "evidence": [{"source": "config", "path": p} for p in cfg_ev]
                + [{"source": "openapi", "path": p} for p in openapi_ev]
                + [{"source": "code", "path": p} for p in code_ev]
                + [{"source": "dependencies", "path": p} for p in dep_ev],
            }
        )
        ...
        relations.append(
            {
                ...
                "evidence": [{"source": "openapi", "path": p} for p in openapi_ev]
                + [{"source": "code", "path": p} for p in code_ev],
                "confidence": round(
                    sum(
                        1
    ...
    if args.diagram:
        mmd = to_mermaid(afm)
        os.makedirs(os.path.dirname(os.path.abspath(args.diagram)), exist_ok=True)
        with open(args.diagram, "w", encoding="utf-8") as f:
            f.write(mmd)
    print(
        f"[archscan] {component_name}  oauth_detected={oauth_detected}  components={len(components)} relations={len(relations)}"
    )
